...
setRatingList(Array.from(RatingSet));
  ...
  return (
    <div className='flex items-center justify-between'>
      ...
      <select className="select select-bordered w-full max-w-xs text-black rounded-lg py-1 border-2 border-blue-400" onChange={(e) => orderDriverList(e.target.value)}>
  <option disabled selected>Price</option>
  <option value={-1}>Min to Max</option>
  <option value={1}>Max to Min</option>
</select>
<select className="select select-bordered md:block py-1 rounded-lg w-full max-w-xs hidden text-black border-2 border-blue-400" onChange={(e)=>setRating(e.target.value)}>
  <option disabled selected>Rating</option>
  {ratingList && ratingList.map((rating:string, index:number)=>(
