...
const Contact = () => {
    ...
    const handleFormSubmit = async (e: { preventDefault: () => void; }) => {
        ...
        if(fullName==''&&email==''&&phone==''&&email==''&&message==''){
            ...
            <div className='container bg-white shadow-xl rounded-2xl border w-full md:w-[60%] border  flex'>
                <div className='w-[60%] flex justify-center items-center flex-col '>
                    ...
                    <form className='px-16 flex flex-col space-y-6 pb-6'>
                        <input value={fullName} onChange={(e)=>{setFullName(e.target.value)}} type="text" placeholder='Full Name' className='pl-1 text-sm w-full h-[60px] border-gray-900 border rounded-md' />
                        <input value={email} onChange={(e)=>{setEmail(e.target.value)}} type="email" placeholder='Email' className='pl-1 text-sm w-full h-[60px]  border-gray-900 border rounded-md' />
                        <input value={phone} onChange={(e)=>{setPhone(e.target.value)}} type="number" placeholder='Phone Number' className='pl-1 text-sm w-full h-[60px]  border-gray-900 border rounded-md' />
                        <textarea value={message} onChange={(e)=>{setMessage(e.target.value)}} placeholder='Write a message here' className='pl-1 pt-1 text-sm w-full h-full border-gray-900 rounded-md border  ' rows={5} cols={50}/>
                        <button
